...
export class JobService {
  ...
  static async toggleJobSaved(jobId: string, currentSavedStatus: boolean): Promise<Job | null> {
    return this.updateJob(jobId, { 
      saved: !currentSavedStatus,
      saved_date: !currentSavedStatus ? new Date().toISOString().split('T')[0] : undefined
    });
  }
  ...
  static async getFilteredJobs(filters: {
    ...
    try {
      ...
      if (filters.searchTerm && filters.searchTerm.trim()) {
        ...
        query = query.or(`title.ilike.%${searchTerm}%,company.ilike.%${searchTerm}%,job_location.ilike.%${searchTerm}%`);
      }
  ...
  static async getJobStatistics(): Promise<{
    ...
    try {
      ...
      return {
        ...
        pending: jobs.filter(job => !job.applied && (!job.status || job.status === 'pending')).length,
  ...
  static async batchUpdateJobs(updates: Array<{ id: string; data: Partial<Job> }>): Promise<Job[]> {
    try {
